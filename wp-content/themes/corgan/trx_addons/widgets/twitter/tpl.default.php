...
$args['columns'] = $args['columns'] < 1 ? $twitter_count : min($args['columns'], $twitter_count);
$args['columns'] = max(1, min(12, (int) $args['columns']));
$args['slider'] = $args['slider'] > 0 && $twitter_count > $args['columns'];
$args['slides_space'] = max(0, (int) $args['slides_space']);

?><div class="widget_content">
	<div class="sc_twitter sc_twitter_<?php
				echo esc_attr($args['type']);
				if ($args['slider']) echo ' swiper-slider-container slider_swiper slider_controls slider_controls_side '.($args['slider_pagination'] > 0 ? 'slider_pagination' : 'slider_nopagination');
				?>"<?php
			echo ($args['columns'] > 1 
	...
	?>><?php
		...
		if (is_array($data) && count($data) > 0) {
			foreach ($data as $tweet) {
				...
				<div class="sc_twitter_item<?php if ($cnt==$twitter_count-1) echo ' last'; ?>">
					...
					<div class="sc_twitter_item_content"><a href="<?php echo esc_url('https://twitter.com/'.trim($twitter_username)); ?>" class="username" target="_blank">@<?php echo esc_html($tweet['user']['screen_name']); ?></a><span class="twitter-text"> <?php
        ...
        if ($args['slider']) {
            ?><div class="slider_controls_wrap"><a class="slider_prev swiper-button-prev" href="#"></a><a class="slider_next swiper-button-next" href="#"></a></div><?php
        }
    ...
    if ($follow) {
        ?><a href="<?php echo esc_url('http://twitter.com/'.trim($twitter_username)); ?>" class="widget_twitter_follow"><?php esc_html_e('Follow us', 'corgan'); ?></a><?php
    }
