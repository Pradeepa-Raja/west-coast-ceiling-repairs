...
function corgan_show_custom_field($id, $field, $value) {
	$output = '';
	switch ($field['type']) {
		...
		case 'mediamanager':
			...
			$title = empty($field['data_type']) || $field['data_type']=='image'
							? esc_html__( 'Choose Image', 'corgan')
							: esc_html__( 'Choose Media', 'corgan');
			$output .= '<a id="'.esc_attr($id).'" class="button mediamanager corgan_media_selector"
				data-param="' . esc_attr($id) . '"
				data-choose="'.esc_attr(isset($field['multiple']) && $field['multiple'] 
					? esc_html__( 'Choose Images', 'corgan') 
					: $title
					).'"
				data-update="'.esc_attr(isset($field['multiple']) && $field['multiple'] 
					? esc_html__( 'Add to Gallery', 'corgan') 
					: $title
					).'"
	...
	return apply_filters('corgan_filter_show_custom_field', $output, $id, $field, $value);
}
...
// $d = corgan_array_merge($a, $b);	["four", "k2"=>"six", "seven", "k1"=>"five");
if (!function_exists('corgan_array_merge')) {
	function corgan_array_merge($a1, $a2) {
...
if (!function_exists('corgan_array_insert')) {
	function corgan_array_insert_after(&$haystack, $needle, $stuff){
		...
		for ($i = 2; $i < func_num_args(); ++$i){
			...
			if (is_array($arg)) {
				if ($i==2)
					...
					$new_array = corgan_array_merge($new_array, $arg);
			} else 
				$new_array[] = $arg;
		}
		...
		$haystack = corgan_array_merge(array_slice($haystack, 0, $i, true), $new_array, array_slice($haystack, $i, null, true));
...
// @param mixed $stuff one or more arrays or scalars to be inserted into $haystack
// @return int the index at which $needle was found
if (!function_exists('corgan_array_before')) {
	function corgan_array_insert_before(&$haystack, $needle, $stuff){
		if (! is_array($haystack) ) return -1;
...
if (!function_exists('corgan_get_tag')) {
	function corgan_get_tag($text, $tag_start, $tag_end) {
		$val = '';
...
if (!function_exists('corgan_html_decode')) {
	function corgan_html_decode($prm) {
...
if (!function_exists('corgan_get_css_position_from_values')) {
	function corgan_get_css_position_from_values($top='',$right='',$bottom='',$left='',$width='',$height='') {
		if (!is_array($top)) {
		...
		foreach ($top as $k=>$v) {
			$imp = substr($v, 0, 1);
			if ($imp == '!') $v = substr($v, 1);
			if ($v != '') $output .= ($k=='width' ? 'width' : ($k=='height' ? 'height' : 'margin-'.esc_attr($k))) . ':' . esc_attr(corgan_prepare_css_value($v)) . ($imp=='!' ? ' !important' : '') . ';';
		}
		return $output;
	}
}
...
if (!function_exists('corgan_prepare_css_value')) {
	function corgan_prepare_css_value($val) {
		if ($val != '') {
			$ed = substr($val, -1);
			if ('0'<=$ed && $ed<='9') $val .= 'px';
		}
...
if (!function_exists('corgan_parse_icons_classes')) {
	function corgan_parse_icons_classes($css) {
		$rez = array();
...
// Minify CSS string
if (!function_exists('corgan_minify_css')) {
	function corgan_minify_css($css) {
		$css = preg_replace("/\r*\n*/", "", $css);
...
// Minify JS string
if (!function_exists('corgan_minify_js')) {
	function corgan_minify_js($js) {
...
// Get GET, POST value
if (!function_exists('corgan_get_value_gp')) {
	function corgan_get_value_gp($name, $defa='') {
		$rez = $defa;
		if (isset($_GET[$name])) {
...
if (!function_exists('corgan_get_value_gps')) {
	function corgan_get_value_gps($name, $defa='', $page='') {
		$putToSession = $page!='';
		$rez = $defa;
		if (isset($_GET[$name])) {
			$rez = stripslashes(trim($_GET[$name]));
		...
		if ($putToSession)
			corgan_set_session_value($name, $rez, $page);
		return $rez;
	}
}

// Get GET, POST, COOKIE value and save it (if need)
if (!function_exists('corgan_get_value_gpc')) {
	function corgan_get_value_gpc($name, $defa='', $page='', $exp=0) {
		$putToCookie = $page!='';
		$rez = $defa;
		if (isset($_GET[$name])) {
			$rez = stripslashes(trim($_GET[$name]));
...
if (!function_exists('corgan_set_session_value')) {
	function corgan_set_session_value($name, $value, $page='') {
...
// Save value into session
if (!function_exists('corgan_del_session_value')) {
	function corgan_del_session_value($name, $page='') {
		if (!session_id()) session_start();
...
if (!function_exists('corgan_hex2rgb')) {
	function corgan_hex2rgb($hex) {
		$dec = hexdec(substr($hex, 0, 1)== '#' ? substr($hex, 1) : $hex);
		return array('r'=> $dec >> 16, 'g'=> ($dec & 0x00FF00) >> 8, 'b'=> $dec & 0x0000FF);
	}
}

if (!function_exists('corgan_hex2rgba')) {
	function corgan_hex2rgba($hex, $alpha) {
		$rgb = corgan_hex2rgb($hex);
		return 'rgba('.intval($rgb['r']).','.intval($rgb['g']).','.intval($rgb['b']).','.floatval($alpha).')';
	}
}

if (!function_exists('corgan_hex2hsb')) {
	function corgan_hex2hsb ($hex) {
		return corgan_rgb2hsb(corgan_hex2rgb($hex));
	}
}

if (!function_exists('corgan_rgb2hsb')) {
	function corgan_rgb2hsb ($rgb) {
		$hsb = array();
...
if (!function_exists('corgan_hsb2rgb')) {
	function corgan_hsb2rgb($hsb) {
		$rgb = array();
		$h = round($hsb['h']);
...
if (!function_exists('corgan_rgb2hex')) {
	function corgan_rgb2hex($rgb) {
		$hex = array(
			dechex($rgb['r']),
			dechex($rgb['g']),
			dechex($rgb['b'])
...
if (!function_exists('corgan_hsb2hex')) {
	function corgan_hsb2hex($hsb) {
		return corgan_rgb2hex(corgan_hsb2rgb($hsb));
	}
}
...
if (!function_exists('corgan_prepare_macros')) {
	function corgan_prepare_macros($str) {
		return str_replace(
			array("{{",  "}}",   "[[",  "]]",   "||"),
			array("<i>", "</i>", "<b>", "</b>", "<br>"),
			$str);
...
if (!function_exists('corgan_remove_macros')) {
	function corgan_remove_macros($str) {
		return str_replace(
			array("{{", "}}", "[[", "]]", "||"),
			array("",   "",   "",   "",   " "),
			$str);
...
if (!function_exists('corgan_is_on')) {
	function corgan_is_on($prm) {
		return $prm>0 || in_array(strtolower($prm), array('true', 'on', 'yes', 'show'));
	}
}
if (!function_exists('corgan_is_off')) {
	function corgan_is_off($prm) {
		return empty($prm) || $prm===0 || in_array(strtolower($prm), array('false', 'off', 'no', 'none', 'hide'));
	}
}
if (!function_exists('corgan_is_inherit')) {
	function corgan_is_inherit($prm) {
		return in_array(strtolower($prm), array('inherit'));		//array('inherit', 'default')
	}
}

// Return truncated string
if (!function_exists('corgan_strshort')) {
	function corgan_strshort($str, $maxlength, $add='...') {
...
if (!function_exists('corgan_unserialize')) {
	function corgan_unserialize($str) {
...
if (!function_exists('corgan_getimagesize')) {
	function corgan_getimagesize($url) {
...
if (!function_exists('corgan_clear_thumb_size')) {
	function corgan_clear_thumb_size($url) {
		$pi = pathinfo($url);
...
if (!function_exists('corgan_add_thumb_size')) {
	function corgan_add_thumb_size($url, $thumb_size, $check_exists=true) {
		$pi = pathinfo($url);
		$parts = explode('-', $pi['filename']);
...
if (!function_exists('corgan_get_thumb_size')) {
	function corgan_get_thumb_size($ts) {
		static $retina = '-';
		if ($retina=='-') $retina = corgan_get_retina_multiplier() > 1 ? '-@retina' : '';
		return ($ts=='post-thumbnail' ? '' : 'corgan-thumb-') . $ts . $retina;
	}
}


// Return url from first <img> tag inserted in post
if (!function_exists('corgan_get_post_image')) {
	function corgan_get_post_image($post_text='', $src=true) {
		global $post;
		$img = '';
...
if (!function_exists('corgan_get_post_audio')) {
	function corgan_get_post_audio($post_text='', $src=true) {
		global $post;
		$img = '';
		if (empty($post_text)) $post_text = $post->post_content;
		if ($src) {
			...
			$img = corgan_get_tag($post_text, '<audio', '</audio>');
			if (empty($mg)) {
				$img = do_shortcode(corgan_get_tag($post_text, '[audio', '[/audio]'));
			}
		}
		return $img;
	}
...
if (!function_exists('corgan_get_post_video')) {
	function corgan_get_post_video($post_text='', $src=true) {
		global $post;
		$img = '';
		if (empty($post_text)) $post_text = $post->post_content;
		if ($src) {
			...
			$img = corgan_get_tag($post_text, '<video', '</video>');
			if (empty($mg)) {
				$img = do_shortcode(corgan_get_tag($post_text, '[video', '[/video]'));
			}
		}
		return $img;
	}
...
if (!function_exists('corgan_get_post_iframe')) {
	function corgan_get_post_iframe($post_text='', $src=true) {
		global $post;
		$img = '';
		if (empty($post_text)) $post_text = $post->post_content;
		if ($src) {
			...
			$img = corgan_get_tag($post_text, '<iframe', '</iframe>');
		return $img;
	}
}
...
if (!function_exists('corgan_make_video_autoplay')) {
	function corgan_make_video_autoplay($video) {
...
if (!function_exists('corgan_enqueue_style')) {	
	function corgan_enqueue_style($handle, $src=false, $depts=array(), $ver=null, $media='all') {
		$load = true;
		...
		if ($load) {
			if (is_array($src) || $src===false) {
				...
				wp_enqueue_style( $handle, esc_url($src).(corgan_is_on(corgan_get_theme_option('debug_mode')) ? (strpos($src, '?')!==false ? '&' : '?').'rnd='.mt_rand() : ''), $depts, $ver, $media );
			}
		}
	}
}
...
if (!function_exists('corgan_enqueue_script')) {	
	function corgan_enqueue_script($handle, $src=false, $depts=array(), $ver=null, $in_footer=true) {
		$load = true;
		if (!is_array($src) && $src !== false && $src !== '') {
			$theme_dir = get_template_directory();
			$theme_url = get_template_directory_uri();
		...
		if ($load) {
			if (is_array($src) || $src===false) {
				...
				wp_enqueue_script( $handle, esc_url($src).(corgan_is_on(corgan_get_theme_option('debug_mode')) ? (strpos($src, '?')!==false ? '&' : '?').'rnd='.mt_rand() : ''), $depts, $ver, $in_footer );
			}
		}
	}
}
...
if (!function_exists('corgan_get_file_dir')) {	
	function corgan_get_file_dir($file, $return_url=false) {
...
if (!function_exists('corgan_get_file_url')) {	
	function corgan_get_file_url($file) {
		return corgan_get_file_dir($file, true);
	}
}

// Detect folder location with same algorithm as file (see above)
if (!function_exists('corgan_get_folder_dir')) {	
	function corgan_get_folder_dir($folder, $return_url=false) {
...
if (!function_exists('corgan_get_folder_url')) {	
	function corgan_get_folder_url($folder) {
		return corgan_get_folder_dir($folder, true);
	}
}


// Add parameters to URL
if (!function_exists('corgan_add_to_url')) {
	function corgan_add_to_url($url, $prm) {
		if (is_array($prm) && count($prm) > 0) {
...
------------------------------------------------------------------- */
if (!function_exists('corgan_init_filesystem')) {
	add_action( 'after_setup_theme', 'corgan_init_filesystem', 0);
	function corgan_init_filesystem() {
...
if (!function_exists('corgan_fpc')) {
    function corgan_fpc($file, $data, $flag=0) {
        ...
        if (!empty($file)) {
            if (isset($wp_filesystem) && is_object($wp_filesystem)) {
                ...
                if (corgan_is_on(corgan_get_theme_option('debug_mode')))
                    throw new Exception(sprintf(esc_html__('WP Filesystem is not initialized! Put contents to the file "%s" failed', 'corgan'), $file));
            }
        }
        return false;
    }
...
if (!function_exists('corgan_fgc')) {	
	function corgan_fgc($file) {
		global $wp_filesystem;
		if (!empty($file)) {
			if (isset($wp_filesystem) && is_object($wp_filesystem)) {
				$file = str_replace(ABSPATH, $wp_filesystem->abspath(), $file);
				...
				if (corgan_is_on(corgan_get_theme_option('debug_mode')))
					throw new Exception(sprintf(esc_html__('WP Filesystem is not initialized! Get contents from the file "%s" failed', 'corgan'), $file));
			}
		}
		return '';
	}
...
if (!function_exists('corgan_fga')) {	
	function corgan_fga($file) {
		global $wp_filesystem;
		if (!empty($file)) {
			if (isset($wp_filesystem) && is_object($wp_filesystem)) {
				$file = str_replace(ABSPATH, $wp_filesystem->abspath(), $file);
				...
				if (corgan_is_on(corgan_get_theme_option('debug_mode')))
					throw new Exception(sprintf(esc_html__('WP Filesystem is not initialized! Get rows from the file "%s" failed', 'corgan'), $file));
			}
		}
		return array();
	}
...
if (!function_exists('corgan_esc')) {	
	function corgan_esc($file) {
