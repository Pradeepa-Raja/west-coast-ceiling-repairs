...
if (have_posts()) {
	...
	if (!$corgan_sticky_out) {
		if (corgan_get_theme_option('first_post_large') && !is_paged() && !in_array(corgan_get_theme_option('body_style'), array('fullwide', 'fullscreen'))) {
			the_post();
	...
	while ( have_posts() ) { the_post(); 
		...
		get_template_part( 'content', $corgan_sticky_out && is_sticky() ? 'sticky' :'classic' );
	}
